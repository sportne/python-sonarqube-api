...
class TestSonarQubeQualityGates(unittest.TestCase):
    ...
    def test_add_group_to_quality_gate(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.add_group_to_quality_gate(gateId="my-gate", groupId="my-group")
    ...
    def test_add_user_to_quality_gate(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.add_user_to_quality_gate(gateId="my-gate", userId="my-user")
            mock_post.assert_called_with(
    ...
    def test_get_application_quality_gate_status(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.get_application_quality_gate_status(application="my-app")
    ...
    def test_copy_quality_gate(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.copy_quality_gate(sourceName="my-gate", destName="new-gate")
            mock_post.assert_called_with(
    ...
    def test_create_quality_gate(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.create_quality_gate(name="my-gate")
            mock_post.assert_called_with(
    ...
    def test_create_quality_gate_condition(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.create_quality_gate_condition(
    ...
    def test_delete_quality_gate_condition(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.delete_quality_gate_condition(id="my-condition")
            mock_post.assert_called_with(
    ...
    def test_deselect_project_from_quality_gate(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.deselect_project_from_quality_gate(projectKey="my-project")
            mock_post.assert_called_with(
    ...
    def test_destroy_quality_gate(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.destroy_quality_gate(name="my-gate")
            mock_post.assert_called_with(
    ...
    def test_get_quality_gate_by_project(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.get_quality_gate_by_project(projectKey="my-project")
            mock_get.assert_called_with(
    ...
    def test_get_project_quality_gate_status(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.get_project_quality_gate_status(projectKey="my-project")
            mock_get.assert_called_with(
    ...
    def test_remove_group_from_quality_gate(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.remove_group_from_quality_gate(
    ...
    def test_remove_user_from_quality_gate(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.remove_user_from_quality_gate(gateId="my-gate", userId="my-user")
            mock_post.assert_called_with(
    ...
    def test_rename_quality_gate(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.rename_quality_gate(currentName="my-gate", newName="new-gate")
            mock_post.assert_called_with(
    ...
    def test_search_quality_gates(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.search_quality_gates(gateId="my-gate")
            mock_get.assert_called_with(
    ...
    def test_search_quality_gate_groups(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.search_quality_gate_groups(gateId="my-gate")
            mock_get.assert_called_with(
    ...
    def test_search_quality_gate_users(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.search_quality_gate_users(gateId="my-gate")
            mock_get.assert_called_with(
    ...
    def test_select_project_for_quality_gate(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.select_project_for_quality_gate(
    ...
    def test_set_quality_gate_ai_code_assurance(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.set_quality_gate_ai_code_assurance(
    ...
    def test_set_default_quality_gate(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.set_default_quality_gate(name="my-gate")
            mock_post.assert_called_with(
    ...
    def test_show_quality_gate(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.show_quality_gate(name="my-gate")
            mock_get.assert_called_with(
    ...
    def test_update_quality_gate_condition(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.update_quality_gate_condition(
