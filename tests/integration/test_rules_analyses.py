...
def test_project_analyses_search(sonarqube_client):
    ...
    try:
        sonarqube_client.projects.create_project(project_key=project_key, name="Test Project Analyses")
        response = sonarqube_client.project_analyses.search_project_analyses(project=project_key)
        assert response.status_code == 200
