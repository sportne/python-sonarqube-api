...
class TestSonarQubeComponents(unittest.TestCase):
    ...
    def test_get_components_app(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.components.get_components_app(
                component="my-project", branch="main"
            )
    ...
    def test_search_components(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.components.search_components(
                qualifiers="TRK", q="test", p=1, ps=10
            )
            mock_get.assert_called_with(
    ...
    def test_search_components_projects(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.components.search_components_projects(q="test", p=1, ps=10)
            mock_get.assert_called_with(
    ...
    def test_show_component(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.components.show_component(component="my-project", branch="main")
            mock_get.assert_called_with(
    ...
    def test_get_component_suggestions(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.components.get_component_suggestions(q="test")
            mock_get.assert_called_with(
    ...
    def test_get_component_tree(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.components.get_component_tree(
                component="my-project", s="name", asc="true"
            )
            mock_get.assert_called_with(
