...
class TestSonarQubeNewCodePeriods(unittest.TestCase):
    ...
    def test_list_new_code_periods(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.new_code_periods.list_new_code_periods(project="my-project")
    ...
    def test_set_new_code_period(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.new_code_periods.set_new_code_period(
    ...
    def test_show_new_code_period(self):
        with patch.object(self.sonar.session, "get") as mock_get:
            self.sonar.new_code_periods.show_new_code_period(project="my-project")
            mock_get.assert_called_with(
    ...
    def test_unset_new_code_period(self):
        with patch.object(self.sonar.session, "post") as mock_post:
            self.sonar.new_code_periods.unset_new_code_period(project="my-project")
            mock_post.assert_called_with(
