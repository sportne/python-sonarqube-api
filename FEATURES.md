...
- [x] Show source code with line oriented info (`api/sources/lines`)
- [x] Get source code as raw text (`api/sources/raw`)
- [x] Get SCM information of source files (`api/sources/scm`)
- [x] Get source code (`api/sources/show`)
...
- [x] Temporarily changes level of logs (`api/system/change_log_level`)
- [x] Provide liveness of SonarQube (`api/system/liveness`)
- [x] Migrate the database (`api/system/migrate_db`)
- [x] Answers "pong" as plain-text (`api/system/ping`)
- [x] Restarts server (`api/system/restart`)
...
- [x] Add a branch of an application selected in a portfolio (`api/views/add_application_branch`)
- [x] Add an existing portfolio to the structure of another portfolio (`api/views/add_portfolio`)
- [x] Add a project to a portfolio (`api/views/add_project`)
- [x] Add a branch of a project selected in a portfolio (`api/views/add_project_branch`)
- [x] List applications which the user has access to that can be added to a portfolio (`api/views/applications`)
- [x] Create a new portfolio (`api/views/create`)
- [x] Delete a portfolio definition (`api/views/delete`)
- [x] List root portfolios (`api/views/list`)
- [x] Move a portfolio (`api/views/move`)
- [x] List possible portfolio destinations (`api/views/move_options`)
- [x] List portfolios that can be referenced (`api/views/portfolios`)
- [x] List projects manually selected in a portfolio (`api/views/projects`)
- [x] Return projects with a failed quality gate (`api/views/projects_status`)
- [x] Refresh one or all of the portfolios (`api/views/refresh`)
- [x] Remove an application from a portfolio (`api/views/remove_application`)
- [x] Remove a branch of an application selected in a portfolio (`api/views/remove_application_branch`)
- [x] Remove a reference to a portfolio (`api/views/remove_portfolio`)
- [x] Remove a project from a portfolio (`api/views/remove_project`)
- [x] Remove a branch of a project selected in a portfolio (`api/views/remove_project_branch`)
- [x] Search for portfolios (`api/views/search`)
- [x] Set the projects selection mode of a portfolio on manual selection (`api/views/set_manual_mode`)
- [x] Set the projects selection mode of a portfolio to none (`api/views/set_none_mode`)
- [x] Set the projects selection mode of a portfolio on regular expression (`api/views/set_regexp_mode`)
- [x] Set the projects selection mode of a portfolio on unassociated projects in hierarchy (`api/views/set_remaining_projects_mode`)
- [x] Set the projects selection mode of a portfolio on project tags (`api/views/set_tags_mode`)
- [x] Show the details of a portfolio (`api/views/show`)
- [x] Update a portfolio (`api/views/update`)

### Webhooks
- [x] Create a Webhook (`api/webhooks/create`)
- [x] Delete a Webhook (`api/webhooks/delete`)
- [x] Get the recent deliveries (`api/webhooks/deliveries`)
- [x] Get a webhook delivery by its id (`api/webhooks/delivery`)
- [x] Search for webhooks (`api/webhooks/list`)
- [x] Update a Webhook (`api/webhooks/update`)

### Web Services
- [x] List web services (`api/webservices/list`)
- [x] Display web service response example (`api/webservices/response_example`)

### Users Management V2
- [x] Create a user (`api/v2/users-management/users`)
- [x] Search users (`api/v2/users-management/users`)
- [x] Fetch a single user (`api/v2/users-management/users/{id}`)
- [x] Update a user (`api/v2/users-management/users/{id}`)
- [x] Deactivate a user (`api/v2/users-management/users/{id}`)

### Fix Suggestions V2
- [x] Suggest a fix for an issue (`api/v2/fix-suggestions/ai-suggestions`)
- [x] Fetch AI suggestion availability (`api/v2/fix-suggestions/issues/{issueId}`)

### DevOps Platform Translation V2
- [x] Create a SonarQube project (`api/v2/dop-translation/bound-projects`)
- [x] List DevOps Platform Integration settings (`api/v2/dop-translation/dop-settings`)

### Clean Code Policy V2
- [x] Create a custom rule (`api/v2/clean-code-policy/rules`)

### Authorizations V2
- [x] Add a group membership (`api/v2/authorizations/group-memberships`)
- [x] Search group memberships (`api/v2/authorizations/group-memberships`)
- [x] Remove a group membership (`api/v2/authorizations/group-memberships/{id}`)
- [x] Create a new group (`api/v2/authorizations/groups`)
- [x] Search groups (`api/v2/authorizations/groups`)
- [x] Fetch a single group (`api/v2/authorizations/groups/{id}`)
- [x] Update a group (`api/v2/authorizations/groups/{id}`)
- [x] Delete a group (`api/v2/authorizations/groups/{id}`)

### System V2
- [x] Get system health (`api/v2/system/health`)
- [x] Get system liveness (`api/v2/system/liveness`)
- [x] Get migrations status (`api/v2/system/migrations-status`)

### SCA V2
- [x] Get risk reports (`api/v2/sca/risk-reports`)
- [x] Get SBOM reports (`api/v2/sca/sbom-reports`)

### Analysis V2
- [x] Get active rules (`api/v2/analysis/active_rules`)
- [x] Get scanner engine metadata/download (`api/v2/analysis/engine`)
- [x] Get all JREs metadata (`api/v2/analysis/jres`)
- [x] Get JRE metadata/download (`api/v2/analysis/jres/{id}`)
- [x] Get scanner engine version (`api/v2/analysis/version`)
